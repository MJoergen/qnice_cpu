...
--
-- It is synthesised too: test/system.vhd instantiates it in the bitstream as
-- well, so that the CPU's irq_valid_i is driven by real logic there and the
-- hardware interrupt path is placed and timed rather than optimised away. See
-- the comment above i_interrupt in that file. The two checks on the CPU's side
-- of the handshake below are simulation only.
--
-- INTERFACE CONTRACTS
...
architecture synthesis of interrupt is
