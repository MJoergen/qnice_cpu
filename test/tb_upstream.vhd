...
--
-- 2. THE REQUEST PORT. The device presents the three-signal request port
--    src/interrupt/README.md describes, and uses it the strict way: it holds
--    its request until a one-cycle accept, with an address valid whenever the
--    request is. This CPU wants the
--    INT_N/IGRANT_N daisy chain of upstream's doc/int-device.md. The adapter
--    between them is the MIRROR IMAGE of the adaptation layer that README says
--    a QNICE system has to supply: that layer has the chain on its device side
--    and the port on its CPU side, this adapter the other way round. The
--    README's "What the adaptation layer has to do" compares the two. The
--    chaining is left out here: there is one device, so there is no priority,
--    no pass-through and no arbitration.
--
...
begin
   ...
   --
   -- This is the mirror image of the adaptation layer src/interrupt/README.md
   -- says a QNICE system has to put in front of OUR CPU: here the device
   -- speaks the port and the CPU speaks the chain. It also leaves out the
   -- chaining -- there is one device, so there is nothing to arbitrate and
